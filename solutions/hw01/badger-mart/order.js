...
function calculateSubtotal() {
	...
	for (let item of ITEMS) {
		const q = parseFloat(document.getElementById(item + "-quantity").value);
		const p = parseFloat(document.getElementById(item + "-price").innerText);
		subtotal += p * q;
	}
...
document.getElementById("btn-checkout").addEventListener("click", () => {
	...
	// Keep the number to two decimal places resulting in a string
});